...
function OmRangeSlider(inputElement, inputValueStyle) {
  ...
  function initVisuals() {
    ...
    visualSliderButtonStart.innerHTML =
      '<div class="dot" style="position: relative; height: 100%; width: 100%;"><div style="position: absolute; height: 48px; width: 48px; top: 50%; left: 50%; transform: translateY(-50%) translateX(-50%);' +
      (debug ? " " : "") +
      '"></div></div>';
  ...
  function refreshRangeIndicator() {
    ...
    const buttonEndMiddle = Math.round(
      elementRangeWidth - (parseInt(buttonEnd.style.left) + buttonEndWidth / 2)
    );
  ...
  function updateValues() {
    ...
    const isFloat =
      parseFloat(settings.min) % 1 !== 0 || parseFloat(settings.max) % 1 !== 0;
